...
- (BOOL)requestRedirectedToRequest:(NSURLRequest *)redirectedRequest {
  ...
  // "about:blank")
  /* GDW -- standardize paths by removing leading/trailing slashes */
  NSString *requestHost = [requestURL host];
  NSString *requestPath = [[requestURL path] stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"/"]];
  NSString *redirectHost = [redirectURL host];
  NSString *redirectPath = [[redirectURL path] stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"/"]];
    
  BOOL isCallback;
  if (requestHost && requestPath) {
      NSLog(@"*** redirect host = %@", redirectHost);
      NSLog(@"*** request host = %@", requestHost);
      NSLog(@"*** redirect path = %@", redirectPath);
  ...
  // tell the delegate that we did handle this request
    //GDW
    NSLog(@"*** About to return YES from requestRedirectedToRequest");
  return YES;
}
